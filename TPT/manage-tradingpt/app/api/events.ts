...
// 구독 플랜 등록 (관리자)
// - 새로운 플랜을 등록하면 기존 활성 플랜이 자동으로 비활성화됩니다.
export async function createSubscriptionPlan(
  data: SubscriptionPlanCreateRequest
): Promise<ApiResponse<number>> {
  return apiCall('/api/v1/admin/subscription-plans', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}

// 활성 구독 플랜 가격 응답 인터페이스
...
// 활성 구독 플랜 목록 조회
export async function getActiveSubscriptionPlans(): Promise<ApiResponse<SubscriptionPlanPriceResponse[]>> {
  return apiCall('/api/v1/admin/subscription-plans/active', {
    method: 'GET',
  });
}
