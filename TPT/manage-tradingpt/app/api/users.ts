import { apiCall, getIsMockMode, mockData } from './index';
import type { ApiResponse } from './index';
import type { SliceInfo } from './reviews';

interface PendingUser {
  userId: number;
  name: string;
  phone: string;
  createdAt: string;
  status: string;
  uid: string;
}

interface UserStatusResponse {
  success: boolean;
  message?: string;
}

interface GrantTokenResponse {
  success: boolean;
  message?: string;
}

// UID 정보 인터페이스
export interface UidInfo {
  exchangeName: string;
  uid: string;
}

// 회원 검색 결과 인터페이스
export interface SearchedUser {
  userId: number;
  name: string;
  phone: string;
  uid: UidInfo | null;
  status: string;
  requestedAt: string;
  trainerName: string | null;
  token: number;
}

export interface PagedSearchResult {
  content: SearchedUser[];
  totalPages: number;
  totalElements: number;
  size: number;
  number: number;
  first: boolean;
  last: boolean;
  empty: boolean;
}

// 신규 구독 고객 인터페이스
export interface LevelTestInfo {
  status: string | null;
  grade: string | null;
  gradingTrainerName: string | null;
}

export interface NewSubscriptionCustomer {
  customerId: number;
  name: string;
  phoneNumber: string;
  uid: string | null;
  hasAttemptedLevelTest: boolean;
  levelTestInfo: LevelTestInfo | null;
  hasConsultation: boolean;
  assignedTrainerName: string | null;
}

export interface SliceNewSubscriptionCustomerResponse {
  totalCount: number;
  content: NewSubscriptionCustomer[];
  sliceInfo: SliceInfo;
}

// 미구독(무료) 고객 인터페이스
export interface FreeCustomer {
  customerId: number;
  name: string;
  phoneNumber: string;
  uid: string | null;
  primaryInvestmentType: string | null;
  token: number;
  createdAt: string;
}

export interface SliceFreeCustomerResponse {
  totalCount: number;
  content: FreeCustomer[];
  sliceInfo: SliceInfo;
}

// 신규 가입자 목록 조회
export async function getPendingUsers(): Promise<ApiResponse<PendingUser[]>> {
  if (getIsMockMode()) {
    return new Promise((resolve) =>
      setTimeout(() => resolve({ success: true, data: mockData.mockPendingUsers }), 300)
    );
  }
  return apiCall('/api/v1/admin/users/pending', { method: 'GET' });
}

// UID 승인/거절 처리
export async function updateUserStatus(
  userId: number,
  status: 'UID_APPROVED' | 'UID_REJECTED'
): Promise<ApiResponse<UserStatusResponse>> {
  return apiCall(`/api/v1/admin/users/${userId}/status?status=${status}`, {
    method: 'PATCH',
  });
}

// 고객에게 토큰 부여
export async function grantUserToken(
  userId: number,
  tokenCount: number
): Promise<ApiResponse<GrantTokenResponse>> {
  return apiCall(`/api/v1/admin/users/${userId}/token`, {
    method: 'PATCH',
    body: JSON.stringify({ tokenCount }),
  });
}

// UID로 회원 검색 (페이지네이션)
export async function searchUsersByUid(
  uid: string,
  page: number = 0,
  size: number = 20
): Promise<ApiResponse<PagedSearchResult>> {
  return apiCall(`/api/v1/admin/users/search-by-uid?uid=${encodeURIComponent(uid)}&page=${page}&size=${size}`, {
    method: 'GET',
  });
}

// 이름으로 회원 검색 (페이지네이션)
export async function searchUsersByName(
  name: string,
  page: number = 0,
  size: number = 20
): Promise<ApiResponse<PagedSearchResult>> {
  return apiCall(`/api/v1/admin/users/search-by-name?name=${encodeURIComponent(name)}&page=${page}&size=${size}`, {
    method: 'GET',
  });
}

// 신규 구독 고객 목록 조회
export async function getNewSubscriptionCustomers(
  page: number = 0,
  size: number = 20
): Promise<ApiResponse<SliceNewSubscriptionCustomerResponse>> {
  return apiCall(`/api/v1/admin/users/new-subscription-customers?page=${page}&size=${size}`, {
    method: 'GET',
  });
}

// 미구독(무료) 고객 목록 조회
export async function getFreeCustomers(
  page: number = 0,
  size: number = 20,
  sort: string = 'createdAt,desc'
): Promise<ApiResponse<SliceFreeCustomerResponse>> {
  return apiCall(`/api/v1/admin/users/free-customers?page=${page}&size=${size}&sort=${sort}`, {
    method: 'GET',
  });
}

// 특정 유저 UID 값 변경
export async function updateUserUid(
  userId: number,
  newUid: string
): Promise<ApiResponse<void>> {
  return apiCall(`/api/v1/admin/users/${userId}/uid`, {
    method: 'PATCH',
    body: JSON.stringify({ uid: newUid }),
  });
}

// 구독 고객 인터페이스
export interface SubscriptionCustomer {
  customerId: number;
  name: string;
  phoneNumber: string;
  uid: string | null;
  investmentType: 'SWING' | 'DAY' | 'SCALPING' | null;
  membershipLevel: 'BASIC' | 'PREMIUM' | null;
  subscriptionStatus: 'ACTIVE' | 'CANCELLED' | 'PAYMENT_FAILED' | 'EXPIRED' | null;
  subscriptionPlanName: string | null;
  nextBillingDate: string | null;
  currentPeriodEnd: string | null;
  trainerName: string | null;
  lastLevelTestGrade: 'A' | 'B' | 'C' | null;
  lastLevelTestScore: number | null;
  completedLectureCount: number;
  totalLectureProgressCount: number;
  unsubmittedAssignmentCount: number;
  totalAssignmentCount: number;
  lectureCompletionRate: number;
  assignmentSubmissionRate: number;
}

export interface SubscriptionCustomerSliceResponse {
  totalCount: number;
  content: SubscriptionCustomer[];
  sliceInfo: SliceInfo;
}

// 구독 고객 목록 조회
export async function getSubscriptionCustomers(
  page: number = 0,
  size: number = 20,
  myCustomersOnly: boolean = false
): Promise<ApiResponse<SubscriptionCustomerSliceResponse>> {
  return apiCall(`/api/v1/admin/subscriptions/customers?page=${page}&size=${size}&myCustomersOnly=${myCustomersOnly}`, {
    method: 'GET',
  });
}

// 내 담당 고객 인터페이스
export interface MyCustomerListItem {
  customerId: number;
  name: string;
  phoneNumber: string;
  investmentType: 'SWING' | 'DAY' | 'SCALPING' | null;
  membershipLevel: 'BASIC' | 'PREMIUM' | null;
  token: number;
}

export interface MyCustomerListResponse {
  customers: MyCustomerListItem[];
  sliceInfo: SliceInfo;
}

// 내 담당 고객 목록 조회
export async function getMyCustomers(
  page: number = 0,
  size: number = 20
): Promise<ApiResponse<MyCustomerListResponse>> {
  return apiCall(`/api/v1/admin/users/my-customers?page=${page}&size=${size}`, {
    method: 'GET',
  });
}

// 투자유형 이력 인터페이스
export interface InvestmentTypeHistory {
  id: number;
  investmentType: 'SWING' | 'DAY';
  investmentTypeDescription: string;
  startDate: string;
  endDate: string | null;
  createdAt: string;
  updatedAt: string;
  ongoing: boolean;
}

// 특정 고객의 투자유형 이력 조회 (관리자)
export async function getCustomerInvestmentTypeHistories(
  customerId: number
): Promise<ApiResponse<InvestmentTypeHistory[]>> {
  return apiCall(`/api/v1/admin/investment-type-histories/${customerId}`, {
    method: 'GET',
  });
}

// 레벨테스트 성적 추이 인터페이스
export interface LevelTestScoreTrendItem {
  attemptId: number;
  attemptOrder: number;
  testDate: string;
  grade: string;
  totalScore: number;
  gradingTrainerName: string;
  assignedTrainerName: string;
}

// 특정 회원의 레벨테스트 성적 추이 조회
export async function getCustomerLevelTestScoreTrend(
  userId: number
): Promise<ApiResponse<LevelTestScoreTrendItem[]>> {
  return apiCall(`/api/v1/admin/leveltests/attempts/history?userId=${userId}`, {
    method: 'GET',
  });
}

// 회원 하드 딜리트 (관리자 전용)
// 고객 및 모든 연관 데이터를 영구 삭제합니다.
export async function deleteUserHard(
  userId: number
): Promise<ApiResponse<void>> {
  return apiCall(`/api/v1/admin/users/${userId}`, {
    method: 'DELETE',
  });
}
