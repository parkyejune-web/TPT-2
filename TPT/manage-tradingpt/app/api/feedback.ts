...
export async function updateBestFeedbacks(
  ...
  const requestBody: UpdateBestFeedbacksRequest = {
    feedbackIds: feedbackIds.slice(0, 4), // 최대 4개만
  };
...
export function getFeedbackStatusColor(status: FeedbackStatus): string {
  const colors: Record<FeedbackStatus, string> = {
    ...
    FN: 'bg-green-100 text-green-800',
...
// 스크린샷 이미지 정보 응답 DTO
export interface FeedbackRequestScreenShotAttachment {
  imageId: number;
  imageUrl: string;
}
...
export interface FeedbackRequestDetail {
  ...
  // 새로운 이미지 구조 (객체 배열)
  screenshotImages?: FeedbackRequestScreenShotAttachment[];
...
export async function getTrainerWrittenFeedbacks(params?: {
  page?: number;
  size?: number;
}): Promise<ApiResponse<TrainerWrittenFeedbackListResponse>> {
  const queryParams = new URLSearchParams();
  if (params?.page !== undefined) queryParams.append('page', String(params.page));
  if (params?.size !== undefined) queryParams.append('size', String(params.size));

  const query = queryParams.toString();
  return apiCall<TrainerWrittenFeedbackListResponse>(
    `/api/v1/feedback-requests/trainer-written${query ? `?${query}` : ''}`,
    {
      method: 'GET',
    }
  );
...
// 트레이너 작성 피드백 일괄 설정
export interface UpdateTrainerWrittenRequest {
  feedbackRequestIds: number[];
}

export async function updateTrainerWrittenFeedbacks(
  feedbackRequestIds: number[]
): Promise<ApiResponse<void>> {
  const requestBody: UpdateTrainerWrittenRequest = {
    feedbackRequestIds,
  };

  return apiCall<void>('/api/v1/admin/feedback-requests/trainer-written', {
    method: 'PATCH',
    body: JSON.stringify(requestBody),
  });
}
...
// 피드백 요청 삭제 (어드민 전용)
// ROLE_ADMIN만 모든 고객의 피드백 요청 삭제 가능
// 관련 응답(FeedbackResponse)도 함께 삭제됨
export async function deleteFeedbackRequestByAdmin(
  feedbackRequestId: number
): Promise<ApiResponse<void>> {
  return apiCall<void>(`/api/v1/admin/feedback-requests/${feedbackRequestId}`, {
    method: 'DELETE',
  });
}
