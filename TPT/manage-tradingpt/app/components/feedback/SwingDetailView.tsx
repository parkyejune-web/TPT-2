  ...
  return (
    <>
      ...
      <section className="space-y-4">
        ...
        {(data.screenshotUrl || (data.screenshotImageUrls && data.screenshotImageUrls.length > 0)) && (
          <div>
            <label className="text-sm font-medium text-gray-600 block mb-2">스크린샷</label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {data.screenshotImageUrls?.map((url, index) => (
                <div
                  key={index}
                  className="relative w-full h-64 border rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all"
                ...
                <div
                  className="relative w-full h-64 border rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-blue-500 transition-all"
                  onClick={() => setSelectedImage(data.screenshotUrl!)}
                >
                  <Image
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium text-gray-600">추가 매수 횟수</label>
            <p className="text-base">{data.additionalBuyCount ?? 0}회</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600">분할 매도 횟수</label>
            <p className="text-base">{data.splitSellCount ?? 0}회</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium text-gray-600">진입가 (Entry Price)</label>
            <p className="text-base">{data.entryPrice?.toLocaleString(undefined, { maximumFractionDigits: 8 }) || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600">탈출가 (Exit Price)</label>
            <p className="text-base">{data.exitPrice?.toLocaleString(undefined, { maximumFractionDigits: 8 }) || '-'}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-base text-red-600">{data.settingStopLoss?.toLocaleString(undefined, { maximumFractionDigits: 8 }) || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600">설정 익절가</label>
            <p className="text-base text-green-600">{data.settingTakeProfit?.toLocaleString(undefined, { maximumFractionDigits: 8 }) || '-'}</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-sm font-medium text-gray-600">전체자산 기준 P&L</label>
            <p className={`text-2xl font-bold ${
              (data.totalAssetPnl ?? 0) >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
              {(data.totalAssetPnl ?? 0) >= 0 ? '+' : ''}{data.totalAssetPnl ?? 0}%
            </p>
          </div>
          <div>
