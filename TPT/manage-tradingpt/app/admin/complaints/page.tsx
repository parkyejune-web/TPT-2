  ...
  const loadComplaints = async () => {
    ...
    if (response.success && response.data) {
      ...
      const formattedComplaints = complaintList.map((complaint: any) => ({
        ...
        customerName: complaint.customerName || complaint.userName || complaint.name || '고객',
        uid: complaint.uid || '',
        ...
        reply: complaint.complaintReply || complaint.reply || complaint.replyContent || '',
        replyAuthor: complaint.answerWriter || complaint.replyAuthor || complaint.replyWriterName || '',
        replyCreatedAt: complaint.answeredAt || complaint.replyCreatedAt || complaint.replyDate || '',
        createdAt: complaint.createdAt || complaint.complaintDate || new Date().toISOString(),
        answered: complaint.answered ?? (complaint.complaintReply || complaint.reply ? true : false),
      }));
      ...
      {showDetailModal && selectedComplaint && (
        <CustomModal title="민원 상세" onClose={handleCloseModal} size="xl">
            <div className="flex justify-end mb-4">
              ...
              >
                {selectedComplaint.answered ? '답변완료' : '대기중'}
              </span>
            </div>
