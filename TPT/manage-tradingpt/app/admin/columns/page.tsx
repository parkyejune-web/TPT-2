...
export default function ColumnsManagementPage() {
  ...
  const handleToggleBest = async (columnId: number, isBest: boolean) => {
    // isBest가 true면 현재 베스트 상태이므로 해제 API 호출
    // isBest가 false면 현재 일반 상태이므로 지정 API 호출
    const response = isBest
      ? await columnsApi.unsetBestColumn(columnId)
      : await columnsApi.setBestColumn(columnId);

    if (response.success) {
      alert(isBest ? '베스트 지정이 해제되었습니다.' : '베스트 칼럼으로 지정되었습니다.');
      loadColumns();
    } else {
      alert(`베스트 칼럼 ${isBest ? '해제' : '지정'} 실패: ${response.error}`);
    }
  };
