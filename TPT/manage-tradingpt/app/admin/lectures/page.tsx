...
import type { Lecture, LectureDetail, Chapter, LectureRequestDTO } from '../../api/lectures';
...
export default function LecturesManagementPage() {
  ...
  const loadChapters = async () => {
    const response = await lecturesApi.getAllChapters();
    if (response.success && response.data) {
      // ChapterListResponse를 Chapter 형태로 변환
      const chaptersData: Chapter[] = response.data.map((item, index) => ({
        chapterId: item.chapterId,
        title: item.title,
        description: '',
        chapterType: item.chapterType || 'REGULAR',
        chapterOrder: index + 1,
        createdAt: '',
      }));
      setChapters(chaptersData);
    } else {
      console.error('챕터 목록 로드 실패:', response.error);
      setChapters([]);
    }
  };

  // 강의 생성
  const handleCreateLecture = async (data: LectureRequestDTO) => {
  ...
  // 강의 수정
  const handleUpdateLecture = async (data: LectureRequestDTO) => {
  ...
  // 챕터 수정
  const handleUpdateChapter = async (chapterId: number, data: {
    title: string;
    description: string;
    chapterType: 'REGULAR' | 'PRO';
    chapterOrder: number;
  }) => {
    const response = await lecturesApi.updateChapter(chapterId, data);
    if (response.success) {
      alert('챕터가 수정되었습니다.');
      loadChapters();
      loadLectures();
    } else {
      alert(`챕터 수정 실패: ${response.error}`);
      throw new Error(response.error);
    }
  };
