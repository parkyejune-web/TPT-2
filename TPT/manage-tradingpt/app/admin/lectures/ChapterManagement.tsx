...
interface ChapterManagementProps {
  ...
  onUpdate: (chapterId: number, data: {
    title: string;
    description: string;
    chapterType: ChapterType;
    chapterOrder: number;
...
export default function ChapterManagement({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      console.log('챕터 생성 요청 데이터:', formData);
      console.log('chapterType:', formData.chapterType);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdate(editingChapter.chapterId, formData);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 챕터 수정 모달 */}
      {editingChapter && (
        <CustomModal title="챕터 수정" onClose={handleEditCancel}>
