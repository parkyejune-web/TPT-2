...
import { getSubscriptionCustomers, deleteUserHard, updateUserUid, type SubscriptionCustomer } from '../../api/users';
...
import LevelTestScoreTrendModal from './LevelTestScoreTrendModal';
...
export default function SubscriptionCustomersTable({ showDeleteButton = false }: Props) {
  ...
  const [historyCustomer, setHistoryCustomer] = useState<SubscriptionCustomer | null>(null);

  // 레벨테스트 성적 추이 모달
  const [showScoreTrendModal, setShowScoreTrendModal] = useState(false);
  const [scoreTrendCustomer, setScoreTrendCustomer] = useState<SubscriptionCustomer | null>(null);

  // 매매일지 내역 모달
  const [showTradingHistoryModal, setShowTradingHistoryModal] = useState(false);
  const [tradingHistoryCustomer, setTradingHistoryCustomer] = useState<SubscriptionCustomer | null>(null);

  // 삭제 모달
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [deleteCustomer, setDeleteCustomer] = useState<SubscriptionCustomer | null>(null);
  ...
  const [uidCustomer, setUidCustomer] = useState<SubscriptionCustomer | null>(null);
  ...
  const loadCustomers = async (pageNum: number) => {
    setLoading(true);
    try {
      const response = await getSubscriptionCustomers(pageNum, 20, false);
      if (response.success && response.data) {
        if (pageNum === 0) {
      ...
      console.error('구독 고객 목록 조회 오류:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getInvestmentTypeBadge = (type: string | null) => {
    if (!type) return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">미설정</span>;
    const typeMap: Record<string, { text: string; color: string }> = {
      'SWING': { text: '스윙', color: 'bg-blue-100 text-blue-800' },
      'DAY': { text: '데이', color: 'bg-green-100 text-green-800' },
      'SCALPING': { text: '스켈핑', color: 'bg-purple-100 text-purple-800' },
    };
  ...
  const getMembershipBadge = (level: string | null) => {
    if (!level) return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">-</span>;
    const levelMap: Record<string, { text: string; color: string }> = {
      'BASIC': { text: 'Basic', color: 'bg-gray-100 text-gray-800' },
      'PREMIUM': { text: 'Premium', color: 'bg-yellow-100 text-yellow-800' },
    };
    const badge = levelMap[level] || { text: level, color: 'bg-gray-100 text-gray-600' };
    return <span className={`px-2 py-1 text-xs rounded-full ${badge.color}`}>{badge.text}</span>;
  };
  ...
  const getStatusBadge = (status: string | null) => {
    ...
    const statusMap: Record<string, { text: string; color: string }> = {
      ...
      'PAYMENT_FAILED': { text: '결제실패', color: 'bg-orange-100 text-orange-800' },
  ...
  const getGradeBadge = (grade: string | null) => {
    if (!grade) return <span className="text-gray-400">미응시</span>;
    const gradeMap: Record<string, string> = {
      'A': 'bg-green-100 text-green-800',
      'B': 'bg-blue-100 text-blue-800',
      'C': 'bg-yellow-100 text-yellow-800',
    };
    return <span className={`px-2 py-1 text-xs rounded-full ${gradeMap[grade] || 'bg-gray-100 text-gray-600'}`}>{grade}</span>;
  };
  ...
  const openHistoryModal = (customer: SubscriptionCustomer) => {
    setHistoryCustomer(customer);
    setShowHistoryModal(true);
  };

  const openScoreTrendModal = (customer: SubscriptionCustomer) => {
    setScoreTrendCustomer(customer);
    setShowScoreTrendModal(true);
  };

  const openTradingHistoryModal = (customer: SubscriptionCustomer) => {
    setTradingHistoryCustomer(customer);
    setShowTradingHistoryModal(true);
  };

  const openDeleteModal = (customer: SubscriptionCustomer) => {
    setDeleteCustomer(customer);
    setShowDeleteModal(true);
  };

  const openUidModal = (customer: SubscriptionCustomer) => {
  ...
  const handleTrainerChange = async (customerId: number, trainerId: number) => {
    ...
    try {
      ...
      if (response.success) {
        // 고객 목록에서 해당 고객의 트레이너 정보 업데이트
        const selectedTrainer = trainers.find((t) => t.trainerId === trainerId);
        setCustomers((prev) =>
          prev.map((c) =>
            c.customerId === customerId
              ? { ...c, trainerName: selectedTrainer?.name || null }
              : c
          )
        );
        alert('트레이너가 변경되었습니다.');
  ...
  return (
    <section className="mb-12">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading && customers.length === 0 ? (
                <tr>
                  <td colSpan={10 + (isAdmin ? 1 : 0) + (showDeleteButton ? 1 : 0)} className="text-center py-6 text-gray-500">
                    로딩 중...
                  </td>
                </tr>
              ) : customers.length === 0 ? (
                ...
                customers.map((customer) => (
                  <tr key={customer.customerId} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm">{customer.customerId}</td>
                    <td className="px-4 py-3 text-sm font-mono text-gray-600">{customer.uid || '-'}</td>
                    ...
                    <td className="px-4 py-3 text-sm">
                      <div className="relative" ref={openDropdownId === customer.customerId ? dropdownRef : null}>
                        ...
                        >
                          <span>{changingTrainer === customer.customerId ? '변경 중...' : customer.trainerName || '미배정'}</span>
                                ...
                                trainers.map((trainer) => (
                                  <button
                                    ...
                                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${
                                      customer.trainerName === trainer.name ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'
                                    }`}
                                  >
                                    {trainer.name}
                                    {customer.trainerName === trainer.name && (
                    ...
                    <td className="px-4 py-3 text-sm">{getGradeBadge(customer.lastLevelTestGrade)}</td>
                    <td className="px-4 py-3 text-sm">
                      <CustomButton
                        variant="secondary"
                        onClick={() => openScoreTrendModal(customer)}
                      >
                        보기
                      </CustomButton>
                    </td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap">
                      <span className="text-blue-600">{customer.completedLectureCount ?? 0}</span>
                      <span className="text-gray-400">/{customer.totalLectureProgressCount ?? 0}</span>
                      <span className="text-gray-500 ml-1">({(customer.lectureCompletionRate ?? 0).toFixed(0)}%)</span>
                    </td>
                    <td className="px-4 py-3 text-sm whitespace-nowrap">
                      <span className="text-green-600">{(customer.totalAssignmentCount ?? 0) - (customer.unsubmittedAssignmentCount ?? 0)}</span>
                      <span className="text-gray-400">/{customer.totalAssignmentCount ?? 0}</span>
                      <span className="text-gray-500 ml-1">({(customer.assignmentSubmissionRate ?? 0).toFixed(0)}%)</span>
                    </td>
                    {isAdmin && (
                      <td className="px-4 py-3 text-sm">
                        <CustomButton
      ...
      {/* 레벨테스트 성적 추이 모달 */}
      {showScoreTrendModal && scoreTrendCustomer && (
        <LevelTestScoreTrendModal
          customerId={scoreTrendCustomer.customerId}
          customerName={scoreTrendCustomer.name}
          onClose={() => setShowScoreTrendModal(false)}
        />
      )}
