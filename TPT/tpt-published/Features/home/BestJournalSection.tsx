...
import { getFeedbackList, FeedbackCardItem } from "../../Shared/api/services/feedbackService";

export function BestJournalSection() {
  const router = useRouter();
  const { checkAccess } = useAccessControl();
  const [feedbacks, setFeedbacks] = useState<FeedbackCardItem[]>([]);
  ...
  useEffect(() => {
    const fetchBestFeedbacks = async () => {
      try {
        setLoading(true);
        // 실시간 트레이딩 목록 조회 후 isBestFeedback 필터링
        const response = await getFeedbackList(0, 50);
        if (response.success && response.data) {
          // isBestFeedback이 true인 항목만 필터링
          const bestFeedbacks = (response.data.feedbacks || []).filter(
            (feedback) => feedback.isBestFeedback === true
          );
          setFeedbacks(bestFeedbacks);
        } else {
          setError(response.message || '베스트 매매일지를 불러오는데 실패했습니다.');
        }
      } catch (err) {
        console.error('[BestJournalSection] 에러:', err);
        setError('베스트 매매일지를 불러오는데 실패했습니다.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="bg-white rounded-lg shadow-sm overflow-hidden border-2 border-yellow-200 animate-pulse">
  ...
  if (error) {
    return (
      <section className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-6">TPT BEST 매매일지</h2>
      ...
      <div className="relative">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
          {displayedFeedbacks.length > 0 ? displayedFeedbacks.map((feedback, index) => (
            <div
              key={`best-feedback-${feedback.feedbackRequestId}-${index}`}
              onClick={() => handleCardClick(feedback.feedbackRequestId)}
            ...
            >
              ...
              <div className="relative h-48">
                ...
                <div className="absolute top-2 left-2">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                      <linearGradient id={`crownGradient-${feedback.feedbackRequestId}`} x1="0%" y1="0%" x2="100%" y2="0%">
                    ...
                    <path
                      ...
                      fill={`url(#crownGradient-${feedback.feedbackRequestId})`}
                    />
                  </svg>
                </div>
              </div>
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-600 mb-2 line-clamp-2">
                  {feedback.contentPreview?.substring(0, 50) || '매매일지 내용'}
                </p>
