...
/**
 * 서버에서 반환되는 사용자 정보 타입
 * NOTE: PAID_BEFORE_TEST, PAID_AFTER_TEST_TRAINER_ASSIGNING 상태 삭제됨
 * - 레벨테스트가 선택 응시로 변경되어 levelTestStatus 필드 추가
 */
interface ServerUserData {
  userId?: number; // 사용자 PK ID (API 응답 필드명)
  name: string;
  username: string;
  email: string;
  ...
  uid?: string | null;
  userStatus?: 'UID_REVIEW_PENDING' | 'UID_REJECTED' | 'UID_APPROVED' | 'TRAINER_ASSIGNED';
  levelTestStatus?: 'AVAILABLE' | 'BEFORE_GRADE' | 'COMPLETED'; // 레벨테스트 상태 (유료 회원 전용)
  leveltestStatus?: 'AVAILABLE' | 'BEFORE_GRADE' | 'COMPLETED'; // API 응답 필드명 (소문자 t)
}
...
const transformServerUserData = (serverData: ServerUserData): User => {
  const { myProfileImage, leveltestStatus, userId, ...rest } = serverData;
  return {
    ...rest,
    id: userId, // userId -> id 변환
    profileImage: myProfileImage, // myProfileImage -> profileImage 변환
    levelTestStatus: rest.levelTestStatus || leveltestStatus, // leveltestStatus(소문자) -> levelTestStatus(대문자) 변환
  };
};
...
export const useAuth = () => {
  ...
  /**
   * 피드백 요청 (DAY/SWING 통합 API)
   * 2025-11-28 기준 새로운 통합 API
   */
  const requestFeedback = useCallback(async (data: FormData): Promise<ApiResponse<any>> => {
    setIsLoading(true);
    try {
      return await feedbackService.createFeedback(data);
    } catch (error) {
      console.error('피드백 요청 오류:', error);
      return { success: false, error: '피드백 요청 중 오류가 발생했습니다.' };
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  /**
   * @deprecated 스윙 피드백 요청 - requestFeedback 사용 권장
   */
  ...
  /**
   * @deprecated 데이 트레이딩 피드백 요청 - requestFeedback 사용 권장
   */
  ...
  /**
   * @deprecated 스켈핑 피드백 요청 - 스켈핑 투자 유형 삭제됨
   */
  ...
  return {
    ...
    updateProfileImage,
    requestFeedback,
    // deprecated - 아래 함수들은 requestFeedback 사용 권장
