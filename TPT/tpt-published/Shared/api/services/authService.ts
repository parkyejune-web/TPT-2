...
/**
 * 비밀번호 재설정 (비로그인 상태)
 * API 명세: PasswordUpdateRequestDTO
 * - username: 사용자 아이디 (필수)
 * - email: 이메일 (필수)
 * - code: 이메일 인증코드 (필수)
 * - newPassword: 새 비밀번호 (필수, 패턴: 숫자+특수문자 포함 6~11자)
 * - newPasswordCheck: 새 비밀번호 확인 (필수)
 */
export const updatePassword = async (
  ...
  console.log('[authService] 비밀번호 재설정 요청 데이터:', { ...data, newPassword: '***', newPasswordCheck: '***' });
  const result = await fetcher<void>(API_ENDPOINTS.AUTH.PASSWORD_UPDATE, {
    method: 'PUT',
    body: JSON.stringify(data),
  });
  console.log('[authService] 비밀번호 재설정 결과:', result);
  return result;
};

/**
 * 비밀번호 변경 (로그인 상태)
 * API 명세: ChangePasswordRequestDTO
...
export const changePassword = async (
  ...
  console.log('[authService] 비밀번호 변경 요청 데이터:', { currentPassword: '***', newPassword: '***' });
  const result = await fetcher<void>(API_ENDPOINTS.USER.CHANGE_PASSWORD, {
    method: 'POST',
    body: JSON.stringify(data),
  });
  console.log('[authService] 비밀번호 변경 결과:', result);
  return result;
};
 ...
 * 닉네임 등록/변경
 */
export const updateNickname = async (data: UpdateNicknameRequestDTO): Promise<ApiResponse<UpdateNicknameResponseDTO>> => {
  console.log('[authService] 닉네임 변경 요청:', data);
  const result = await fetcher<UpdateNicknameResponseDTO>(API_ENDPOINTS.USER.NICKNAME, {
    method: 'POST',
    body: JSON.stringify(data),
  });
  console.log('[authService] 닉네임 변경 응답:', result);
  return result;
};
