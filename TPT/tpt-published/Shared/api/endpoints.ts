...
export const API_ENDPOINTS = {
  ...
  USER: {
    ...
    NICKNAME: '/api/v1/user/nickname', // 닉네임 등록/변경
  },

  // ==================== 피드백 요청 ====================
  FEEDBACK_REQUEST: {
    // 피드백 생성 (DAY/SWING 통합 API)
    CREATE: '/api/v1/feedback-requests',
    // Legacy endpoints (deprecated - 스켈핑 삭제됨)
    ...
    TRAINER_WRITTEN: '/api/v1/feedback-requests/trainer-written', // 트레이너 작성 매매일지 (무한 스크롤)
    DETAIL: (id: number) => `/api/v1/feedback-requests/${id}`,
    UPDATE: (id: number) => `/api/v1/feedback-requests/${id}`,
  ...
  WEEKLY_TRADING: {
    ...
    /** @deprecated - UPSERT_MEMO 사용 권장 */
    CREATE: (year: number, month: number, week: number) =>
      `/api/v1/weekly-trading-summary/customers/me/years/${year}/months/${month}/weeks/${week}`,
    /** 주간 매매일지 메모 Upsert (Customer) - PUT */
    UPSERT_MEMO: (year: number, month: number, week: number) =>
      `/api/v1/weekly-trading-summary/customers/me/years/${year}/months/${month}/weeks/${week}/memo`,
    PROFIT_FEEDBACKS: (year: number, month: number, week: number) =>
      `/api/v1/weekly-trading-summary/customers/me/years/${year}/months/${month}/weeks/${week}/profit-feedbacks`,
    LOSS_FEEDBACKS: (year: number, month: number, week: number) =>
      `/api/v1/weekly-trading-summary/customers/me/years/${year}/months/${month}/weeks/${week}/loss-feedbacks`,
  ...
  REVIEW: {
    ...
    TAGS: '/api/v1/reviews/tags', // 리뷰 태그 목록 조회
    STATISTICS: '/api/v1/reviews/statistics', // 리뷰 통계 조회
  ...
  PAYMENT_METHOD: {
    // 빌링키 등록
    BILLING_KEY_INIT: '/api/v1/payment-methods/billing-key/init',
    BILLING_KEY_REGISTER: '/api/v1/payment-methods/billing-key/register',
    BILLING_KEY_DIRECT: '/api/v1/payment-methods/billing-key/direct',

    // 결제수단 관리
    LIST: '/api/v1/payment-methods',
    DETAIL: (paymentMethodId: number) => `/api/v1/payment-methods/${paymentMethodId}`,
    DELETE: (paymentMethodId: number) => `/api/v1/payment-methods/${paymentMethodId}`,
  },

  // ==================== 구독 플랜 ====================
  SUBSCRIPTION_PLAN: {
    ACTIVE: '/api/v1/subscription-plans/active',
  },
  ...
  LECTURE: {
    ...
    DETAIL: (lectureId: number) => `/api/v1/lectures/${lectureId}`,

    // 강의 재생 URL 발급 (3시간 유효)
    PLAY: (lectureId: number) => `/api/v1/lectures/${lectureId}/play`,

    // 토큰 구매
    PURCHASE: (lectureId: number) => `/api/v1/lectures/${lectureId}/purchase`,

    // 진행도 업데이트
    PROGRESS: (lectureId: number) => `/api/v1/lectures/${lectureId}/progress`,

    // 과제 제출
    SUBMIT_ASSIGNMENT: (lectureId: number) => `/api/v1/lectures/${lectureId}/assignments/submit`,
    MY_ASSIGNMENT: (lectureId: number) => `/api/v1/lectures/${lectureId}/assignments/me`,

    // 첨부파일 다운로드 URL 발급
    ATTACHMENT_DOWNLOAD: (lectureId: number, attachmentId: number) =>
      `/api/v1/lectures/${lectureId}/attachments/${attachmentId}/download`,
  },
} as const;
