...
import { getWeeklyProfitFeedbacks } from '../../../Shared/api/services/feedbackService';
import type { WeeklyFeedbackListItem } from '../../../Shared/api/apiTypes';
import { ChevronLeft, ChevronRight } from 'lucide-react';

/**
 * 주간 수익 매매 모아보기 페이지 - Apple/Toss 수준 프리미엄 디자인
 */
export default function FeedbackWeekProfitPage() {
  ...
  useEffect(() => {
    const fetchProfitFeedbacks = async () => {
      setLoading(true);
      try {
        const weekNumber = getWeekNumber(week);
        const response = await getWeeklyProfitFeedbacks(
          parseInt(year),
          parseInt(month),
          weekNumber
        );
        ...
        if (response.success && response.data) {
          setFeedbacks(response.data.profitFeedbacks);
          setTotalAssetPnlSum(response.data.totalAssetPnlSum ?? 0);
        } else {
          console.error('[FeedbackWeekProfit] 수익 매매 조회 실패:', response.error);
        }
      } catch (error) {
        console.error('[FeedbackWeekProfit] 수익 매매 조회 에러:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchProfitFeedbacks();
  }, [year, month, week]);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12 pb-20">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-12 sm:mb-16">
          ...
          <div className="bg-gray-50 rounded-2xl sm:rounded-3xl p-4 sm:p-6 transition-all hover:bg-gray-100">
            ...
            <p className="text-2xl sm:text-3xl lg:text-4xl font-bold text-green-600 tracking-tight">
