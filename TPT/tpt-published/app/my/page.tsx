 ...
 *    (예: 'UID_REVIEW_PENDING', 'UID_REJECTED', 'UID_APPROVED', 'TRAINER_ASSIGNED')
 *
 * NOTE: PAID_BEFORE_TEST, PAID_AFTER_TEST_TRAINER_ASSIGNING 상태가 삭제됨
 * - 레벨테스트가 선택 응시로 변경되어, isPremium + levelTestStatus로 관리
 */
export default function MyPage() {
  ...
  let levelTestStatus: LevelTestStatus | undefined;

  if (useMockData && mockData) {
    // Mock Data 사용
    userData = mockData;
    userStatus = mockData.userStatus;
    isPremium = mockData.isPremium || false;
    levelTestStatus = mockData.levelTestStatus;
  } else {
    // 실제 사용자 데이터 사용
    if (!user) {
      return null;
    ...
    userData = {
      ...
      remainingToken: user.token ?? user.remainingToken ?? 0,
      paymentMethod: user.paymentMethod,
      levelTestStatus: user.levelTestStatus,
    };
  ...
  return (
    <div className="w-full min-h-screen bg-white flex flex-col justify-center md:flex-row relative sm:px-20 md:px-30">
      ...
      <MyPageMain state={userStatus} isPremium={isPremium} levelTestStatus={levelTestStatus} />
