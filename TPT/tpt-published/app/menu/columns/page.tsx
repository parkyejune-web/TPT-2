...
export default function ColumnsPage() {
  ...
  const [columns, setColumns] = useState<ColumnListResponseDTO[]>([]);
  ...
  const filteredColumns = columns.filter((col) => {
    const categoryMatch = selectedCategory === '전체' || col.categoryName === selectedCategory;
    const trainerMatch = selectedTrainer === '전체' || col.writerName === selectedTrainer;
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        {filteredColumns.length === 0 ? (
          ...
          filteredColumns.map((column) => (
            <div
              key={column.columnId}
              onClick={() => router.push(`/columns/${column.columnId}`)}
            ...
            >
              ...
              <div className="px-4 py-2">
                <span
                  className={`inline-block px-2 py-1 text-xs font-medium rounded ${
                    column.categoryName === '주식'
                      ? 'bg-blue-100 text-blue-700'
                      : column.categoryName === '채권'
              ...
              <div className="px-4 py-3">
                ...
                <div className="flex justify-between items-center text-sm text-gray-600">
                  <span>{column.writerName}</span>
                  <div className="flex gap-4">
                    <span>좋아요 {column.likeCount}</span>
                    <span>댓글 {column.commentCount}</span>
                  </div>
                </div>
              </div>
            </div>
